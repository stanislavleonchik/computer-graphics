...
void make_affine_transforms(Matrix4x4& model, Mesh mesh) {
    ...
    model = Matrix4x4::translate(Point3(-figureCenter.x, -figureCenter.y, -figureCenter.z)) * model; // Применение смещения
    ...
    model = Matrix4x4::translate(figureCenter) * model; // Применение смещения
